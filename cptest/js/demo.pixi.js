...
var Demo = function() {
	// PIXI canvas def
	/*var renderer = Demo.prototype.renderer = new PIXI.autoDetectRenderer(window.innerWidth, window.innerHeight);//PIXI.autoDetectRenderer(256, 256, null);
	var stage = Demo.prototype.stage = new PIXI.DisplayObjectContainer();
	var container = Demo.prototype.container = new PIXI.Stage(0x338833,true);
	var content = document.getElementById('content');
	content.appendChild(this.renderer.view);
	this.container.addChild(stage);
	stage.position.x = renderer.width*0.5; // center at origin
	stage.position.y = -renderer.height*0.5;*/

	//var ctx = Demo.prototype.ctx = renderer.context
...
var drawCirclePixi = function(g, scale, point2canvas, c, radius,angle){//(g,x,y,angle,radius,color,lineWidth){
	var c = point2canvas(c);
    lineWidth = lineWidth || 1;
    color = typeof(color)!="undefined" ? color : 0xffffff;
    g.lineStyle(lineWidth, 0x000000, 1);
    g.beginFill(color, 1.0);
    g.drawCircle(c.x, c.x, radius);
    g.endFill();

    // line from center to edge
    g.moveTo(c.x,c.x);
    g.lineTo(   c.x + radius*Math.cos(-angle),
                c.x + radius*Math.sin(-angle) );
};

var drawRectanglePixi = function(g, point2canvas, pos, size,color,lineWidth){//(g,x,y,angle,w,h,color,lineWidth){
	var pos_ = point2canvas(pos);
	var size_ = cp.v.sub(point2canvas(cp.v.add(pos, size)), pos_);
    lineWidth = lineWidth || 1;
    color = typeof(color)=="undefined" ? 0x000000 : color;
    g.lineStyle(lineWidth, color, 1);
    g.drawRect(pos_.x, pos_.y, size_.x, size_.y);
};
